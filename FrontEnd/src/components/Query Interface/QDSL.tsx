...
function QDSL({getRoute, history}:appProps) {
    ...
    async function getNames(data:any){
         setRoute("query")
          axios.get(`http://localhost:8080/newForm/${data.dataBase}`).then(res=>{
    ...
    async  function getEditProp(edit:any){
       
        axios.get(`http://localhost:8080/getUiEditProperty/${dataBase}/${edit.inputName}`).then(res=>{
            setEdit(res.data)
           
            }
        )
        
        setEditRoute("editProp")
    }
    ...
    async  function editForm(edit:any){
           ...
           try{
          const res = await axios.post(`http://localhost:8080/updateFormData/${dataBase}`, edit)
   ...
   async function getNumericalProperties(name:any, x:any){
    ...
    return (
       ...
       <div className="flex flex-col z-[70] w-[80%] ml-[-42%]  ">
        ...
        <QDataBase getRoute={routeController} route={route}  outPut={getNames}/>
        
        {/* get Names */}
        {
            (!isUndefined(names)&&editRoute==="names")&&
            <Backdrop
            sx={{ color: '#fff', zIndex: (theme) => theme.zIndex.drawer + 1}}
            open={(!isUndefined(names)&&editRoute==="names")} >
        ...
        { /* Edit Form */ }
        {
            (!isUndefined(edit)&&nps===false&&editRoute==="editProp")&&
            <Backdrop
            sx={{ color: '#fff', zIndex: (theme) => theme.zIndex.drawer + 1}}
            open={(!isUndefined(edit)&&nps===false&&editRoute==="editProp")} >
                <div className="grid grid-cols-1 w-[40%]"> 
